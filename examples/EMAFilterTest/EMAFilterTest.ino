...
#define INPUT_PERIOD_SAMPLES 50 // Signal with 50 samples per period to see sines generated by double and triple filters.
...
void loop() {
    ...
    static uint8_t sPeriodCount = 0; // 0 to INPUT_PERIOD_SAMPLES (200)
    ...
    sInput = MAXIMUM_INPUT_VALUE * sin(((float) sPeriodCount * TWO_PI) / INPUT_PERIOD_SAMPLES);

#elif defined(TEST_WITH_TRIANGLE_INPUT)
    if (sPeriodCount < INPUT_PERIOD_SAMPLES / 2) {
        sInput = (((MAXIMUM_INPUT_VALUE * 2) * sPeriodCount) / (INPUT_PERIOD_SAMPLES / 2)) - MAXIMUM_INPUT_VALUE;
    } else {
        sInput = (3 * MAXIMUM_INPUT_VALUE) - (((MAXIMUM_INPUT_VALUE * 2) * sPeriodCount) / (INPUT_PERIOD_SAMPLES / 2));
    }
#else
        if (sPeriodCount < INPUT_PERIOD_SAMPLES / 2) {
        ...
        sPeriodCount++;
        if (sPeriodCount >= INPUT_PERIOD_SAMPLES) {
            sPeriodCount = 0;
        }
