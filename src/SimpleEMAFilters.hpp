 ...
 *
 *  Copyright (C) 2020-21  Armin Joachimsmeyer
...
void doLowpass5_int16(int16_t *aLowpassAccumulator_int16, int16_t aInputValue) {
    *aLowpassAccumulator_int16 += ((aInputValue - *aLowpassAccumulator_int16) + (1 << 4)) >> 5;
}
...
// "+ (1 << 2)" to avoid to much rounding errors and it costs only 1 clock extra
    ...
    sLowpass8_int32 += (((((int32_t) aInputValue) << 8) - sLowpass8_int32) + (1 << 7)) >> 8; //
    timingPinLow();
